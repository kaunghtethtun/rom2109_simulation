...
namespace rom_dynamics
{
  ...
  AvoidanceNode::control_cycle()
  {
    ...
    if (last_scan_ == nullptr || (now() - last_scan_->header.stamp) > 1s) { return; }
    ...
    vel.linear.x = std::clamp(module, 0.0, 1.0);  // truncate linear vel to [0.0, 0.3] m/s

    vel.angular.z = std::clamp(angle, -0.5, 0.5);  // truncate rotation vel to [-0.5, 0.5] rad/s

    vel_pub_->publish(vel);

    RCLCPP_INFO(rclcpp::get_logger("\033[1;31mLineVel\033[1;0m"), ": \033[1;31m%.4f m/s\033[1;0m", vel.linear.x);
    RCLCPP_INFO(rclcpp::get_logger("\033[1;31mAnguVel\033[1;0m"), ": \033[1;31m%.4f m/s\033[1;0m", vel.angular.z);
  ...
  void
  AvoidanceNode::my_control_cycle()
  {
    // Skip cycle if no valid recent scan available
    if (last_scan_ == nullptr || (now() - last_scan_->header.stamp) > 1s) { return; }

    // Get VFF vectors
    const VFFVectors & vff = my_get_vff(*last_scan_);

    // Use result vector to calculate output speed
    const auto & v = vff.result;
    double angle = atan2(v[1], v[0]);                 // Angular displacement i think ( Heading )
    double dist = sqrt(v[0] * v[0] + v[1] * v[1]);  // Lin displacement i think

    // Create ouput message, controlling speed limits
    geometry_msgs::msg::Twist vel;
  
    //vel.linear.x = std::clamp(dist, 0.0, 1.0);  // truncate linear vel to [0.0, 0.3] m/s
    //vel.angular.z = std::clamp(angle, -0.5, 0.5);  // truncate rotation vel to [-0.5, 0.5] rad/s

    static const double scaleRotationRate = 15.0;
    static const double scaleForwardSpeed = 3.0;
    static const double time_t = 20;

    vel.linear.x = (dist/time_t) * scaleForwardSpeed;
    vel.angular.z= (angle/time_t) * scaleRotationRate;
    // ဒါက my_get_vff ထဲက variables 
    //vel.linear.x = std::clamp(distance_max, 0.0, 1.0);  // truncate linear vel to [0.0, 0.3] m/s
    //vel.angular.z = std::clamp(angle, -0.5, 0.5);  // truncate rotation vel to [-0.5, 0.5] rad/s
  ...
  VFFVectors
  AvoidanceNode::my_get_vff(sensor_msgs::msg::LaserScan & scan)
  {
    // This is the obstacle radious in which an obstacle affects the robot
    const float OBSTACLE_DISTANCE = 2.0;
    ...
    for( unsigned int i=0; i<360; i++ )
    {
      ...
      {
        if ( std::isinf(scan.ranges[i]) ) { scan.ranges[i] = 0; }
      }
      else if( i>=270 && i<360 )
      {
        scan.ranges[i] = 0;
    ...
    // Get the index of long path way
    int max_idx = std::max_element(scan.ranges.begin(), scan.ranges.end()) - scan.ranges.begin();

    // Get the distance to nearest obstacle
    double distance_max = scan.ranges[max_idx];

    double angle = scan.angle_min + scan.angle_increment * max_idx;

    // Calculate resulting vector using maximum distance
    vff_vector.result[0] = cos(angle) * distance_max;
    vff_vector.result[1] = sin(angle) * distance_max;
